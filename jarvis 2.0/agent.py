...
def execute_system_command(command):
    ...
    # --- CLOSE SPECIFIC OPEN FOLDER ---
    elif "close folder" in cmd:
        try:
            import wmi
            c = wmi.WMI()
    
            # Step 1: Get list of all open Explorer windows
            explorer_paths = []
            for p in c.Win32_Process(name="explorer.exe"):
                if p.CommandLine and "Explorer.EXE" in p.CommandLine:
                    if "/factory" in p.CommandLine:  # ignore background process
                        continue
                    path_start = p.CommandLine.find('"') + 1
                    path_end = p.CommandLine.rfind('"')
                    folder_path = p.CommandLine[path_start:path_end].strip()
                    if os.path.isdir(folder_path):
                        explorer_paths.append(folder_path)
    
            if not explorer_paths:
                speak("Sir, no folder windows are currently open.")
                return True
    
            # Step 2: Ask which folder to close
            speak("Sir, which folder should I close?")
            folder_name = listen()
            if not folder_name:
                speak("No folder name received. Cancelling.")
                return True
    
            folder_name = folder_name.strip().lower()
    
            # Step 3: Match spoken folder with open folders
            matching_folders = [p for p in explorer_paths if os.path.basename(p).lower() == folder_name]
    
            if not matching_folders:
                speak(f"Sir, folder '{folder_name}' is not open.")
                return True
    
            # Step 4: Kill only that Explorer window
            for p in c.Win32_Process(name="explorer.exe"):
                if p.CommandLine and matching_folders[0] in p.CommandLine:
                    p.Terminate()
                    speak(f"Folder '{folder_name}' has been closed.")
                    return True
    
            speak("Sir, I found the folder but could not close it.")
    
        except Exception as e:
            speak(f"Could not close the folder. Error: {str(e)}")
